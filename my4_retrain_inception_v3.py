...
##############################################################################
console_log = open('console4_log' + str(TIME_NOW) + '.txt', 'w')
misscls_test_list = open('misscls4_test_list' + str(TIME_NOW) + '.txt', 'w')
##############################################################################
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  with tf.name_scope('accuracy'):
    with tf.name_scope('correct_prediction'):
      ...
      correct_prediction5 = tf.nn.in_top_k(result_tensor, tf.argmax(ground_truth_tensor, 1), 5)
    with tf.name_scope('accuracy'):
      evaluation_step = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
      evaluation_step5 = tf.reduce_mean(tf.cast(correct_prediction5, tf.float32))
  tf.summary.scalar('accuracy', evaluation_step)
  tf.summary.scalar('accuracy5', evaluation_step5)
  return evaluation_step, evaluation_step5, prediction, correct_prediction5
...
def main(_):
  ...
  summaries_dir_now = FLAGS.summaries_dir + str(TIME_NOW)
  tf.gfile.MakeDirs(summaries_dir_now)
  ...
  with tf.Session(graph=graph) as sess:
    ...
    evaluation_step, evaluation_step5, prediction, correct_prediction5 = add_evaluation_step(
        final_tensor, ground_truth_input)

    # 모든 summaries를 합치고(merge) summaries_dir에 쓴다.(write)
    merged = tf.summary.merge_all()
    train_writer = tf.summary.FileWriter(summaries_dir_now + '/train',
                                         sess.graph)

    validation_writer = tf.summary.FileWriter(
        summaries_dir_now + '/validation')

    # 우리의 모든 가중치들(weights)과 그들의 초기값들을 설정한다.
    init = tf.global_variables_initializer()
    sess.run(init)
    with gfile.FastGFile(FLAGS.output_labels + str(TIME_NOW) + '.txt', 'w') as qq:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        train_accuracy, train_accuracy5, cross_entropy_value, correct_predictions5 = sess.run(
            [evaluation_step, evaluation_step5, cross_entropy, correct_prediction5],
            feed_dict={bottleneck_input: train_bottlenecks,
                       ground_truth_input: train_ground_truth})
        print('Step %d: Train accuracy : T1 = %.1f%% , T5 = %.1f%%' % (i, train_accuracy * 100, train_accuracy5 * 100))
        print('Step %d: Cross entropy = %f' % (i, cross_entropy_value))
        console_log.write('Step %d: Train accuracy : T1 = %.1f%% , T5 = %.1f%%\n' % (i, train_accuracy * 100, train_accuracy5 * 100))
        console_log.write('Step %d: Cross entropy = %f\n' % (i, cross_entropy_value))
        ...
        # TensorBoard를 위한 'merged' op을 이용해서 training summaries을 capture한다.
        validation_summary, validation_accuracy, validation_accuracy5, correct_predictions5 = sess.run(
            [merged, evaluation_step, evaluation_step5, correct_prediction5],
            feed_dict={bottleneck_input: validation_bottlenecks,
                       ground_truth_input: validation_ground_truth})
        validation_writer.add_summary(validation_summary, i)
        print('Step %d: Validation accuracy : T1 = %.1f%% , T5 = %.1f%% (N=%d)' %
              (i, validation_accuracy * 100, validation_accuracy5 * 100, len(validation_bottlenecks)))
        console_log.write('Step %d: Validation accuracy : T1 = %.1f%% , T5 = %.1f%% (N=%d)\n' %
              (i, validation_accuracy * 100, validation_accuracy5 * 100, len(validation_bottlenecks)))
        console_log.flush()
      if i % 5000 == 0:
...
#############################
    test_accuracy, test_accuracy5, predictions, correct_predictions5 = sess.run(
        [evaluation_step, evaluation_step5, prediction, correct_prediction5],
        feed_dict={bottleneck_input: test_bottlenecks,
                   ground_truth_input: test_ground_truth})
    print('Final test accuracy : T1 = %.1f%% , T5 = %.1f%% (N=%d)' % (test_accuracy * 100, test_accuracy5 * 100, len(test_bottlenecks)))
    console_log.write('Final test accuracy : T1 = %.1f%% , T5 = %.1f%% (N=%d)\n' % (test_accuracy * 100, test_accuracy5 * 100, len(test_bottlenecks)))
    console_log.close()

    if FLAGS.print_misclassified_test_images:
      print('=== Writing MISCLASSIFIED TEST IMAGES ===')
      for i, test_filename in enumerate(test_filenames):
        if correct_predictions5[i] == False:
          misscls_test_list.write('%70s  %s\n' % (test_filename, list(image_lists.keys())[predictions[i]]))
      misscls_test_list.close()
      print('=== End writing ===')

    # 학습된 graph와 weights들을 포함한 labels를 쓴다.(write)
    output_graph_def = graph_util.convert_variables_to_constants(
        sess, graph.as_graph_def(), [FLAGS.final_tensor_name])
    with gfile.FastGFile(FLAGS.output_graph + '.pb', 'wb') as f:
      f.write(output_graph_def.SerializeToString())
    with gfile.FastGFile(FLAGS.output_labels + '.txt', 'w') as f:
      f.write('\n'.join(image_lists.keys()) + '\n')
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--output_graph',
      type=str,
      default='./my_retr_res/logs/test4/output_graph',
      help='Where to save the trained graph.'
  )
  parser.add_argument(
      '--output_labels',
      type=str,
      default='./my_retr_res/logs/test4/output_labels',
      help='Where to save the trained graph\'s labels.'
  )
  parser.add_argument(
      '--summaries_dir',
      type=str,
      default='./my_retr_res/logs/test4/retrain_logs',
  ...
  parser.add_argument(
      '--bottleneck_dir',
      type=str,
      default='./my_retr_res/bottleneck',
